...
using namespace std;
 ...
 class stos
 {
	...
	protected:
 	...
 	typ check()
 	{
 		if(l->next == NULL){ cout<<"Stos jest pusty"; return 0;}else
 ...
 {
 	if(a==b || ((a=='+' ||a=='-')&&(b=='+' ||b=='-')) || ((a=='*' || a=='/')&&(b=='*' || b=='/'))   ||	((a=='+' || a=='-') && (b=='*' || b=='/' || b=='^' || b=='~'))  || (((a=='*')||a=='/')&&(b=='^' || b=='~')) || ((b=='~')&&(a=='^' )))
 	{return true;}else
 	return false;
...
int main()
{
	...
	stos cc;
 	...
 	queue qu;
 	cc.put(1); cc.put(2); cc.put(3); cc.put(4);
 	//cout<<"Oto aktualny stos: "<<endl;
 	
	string ww="2+3+5";
	const char* j=ww.c_str();
	j=uzysk(j, x);
	j=uzysk(j, y);
	cout<<"Oto uzysk: "<<x<<" Oraz y: "<<y<<endl;
	
	string aa="2+3.3*(2+6.2*(5+6.6))+5";
	string gg;
	gg= tonp(aa);
	x=onpwynik(gg);
